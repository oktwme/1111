...
function Jinx:spell_check()
    ...
    self.q_as_buff = utils:get_buff_count(player, "jinxqramp")
    self.q_mini_gun = utils:has_buff(player, "jinxqicon")
    self.q_big_gun = utils:has_buff(player, "jinxq")
...
function Jinx:pred_check(target)
        ...
        if obj and not obj.isZombie then
            ...
            elseif self.w_ready then
                ...
                if filter_w() and cast_pos ~= vec3(0, 0, 0) then
                    if mymenu.auto.w_ks:get() and dmg_lib:Jinx_W(player, obj) > utils:get_real_hp(obj, true, true)
                        and
                        utils:is_valid(obj)
                        and
            ...
            then
                ...
                local function filter_r()
                    ...
                    if damagelib.get_spell_damage('JinxR', 0, player, obj, false, 0) < utils:get_real_hp(obj, true, true) then return false end
...
function Jinx:W_logic()
    ...
    if aa_damage > dmg_lib:Jinx_W(player, target) and in_aa then return end
    ...
    local pred_3 = Jinx:pred_check(target) and utils:get_cc_time(target) == 0
...
function Jinx:Steal_monster()
    ...
    for i = 0, Obj_size - 1 do
        ...
        local function filter()
            ...
            local can_kill = dmg_lib:Jinx_W(player, obj) > obj_hp
            if can_kill then
                return true
            end
            return false
...
function Jinx:R_logic()
    ...
    local pred_3 = Jinx:pred_check(target)
    local cast_pos = vec3(seg.endPos.x, target.pos.y, seg.endPos.y)
...
function Jinx:on_process_spell_cast(spell)
    if not spell or player.isDead then return end

    if spell.owner and spell.owner == player and spell.name == "JinxQ" then
        self.last_q_t = game.time
    end
end

--#region draw
function Jinx:draw_damage()
    ...
    do
        ...
        then
            ...
            if mymenu.dr.w_damage:get() and self.w_ready then
                dmg.w = dmg_lib:Jinx_W(player, enemy)
            end
            if mymenu.dr.r_damage:get() and self.r_ready then
                dmg.r = damagelib.get_spell_damage('JinxR', 0, player, enemy, false, 0)
            end
...
local function Jinx_on_tick()
    Jinx:on_tick()
end

local function Jinx_on_draw()
    Jinx:on_draw()
end

local function Jinx_on_process_spell(spell)
    Jinx:on_process_spell_cast(spell)
end

local function Jinx_path(target)
...
function Jinx:bind_callbacks(b_remove)
    if not b_remove
    then
        ...
        cb.add(cb.tick, Jinx_on_tick)
        cb.add(cb.draw, Jinx_on_draw)
        cb.add(cb.spell, Jinx_on_process_spell)
        cb.add(cb.path, Jinx_path)

        self.b_callbacks_registered = true
    elseif b_remove and b_remove == true
    then
        cb.remove(cb.tick, Jinx_on_tick)
        cb.remove(cb.draw, Jinx_on_draw)
        cb.remove(cb.spell, Jinx_on_process_spell)
        cb.remove(cb.path, Jinx_path)

        self.b_callbacks_registered = false
    end
end

function Jinx:init(b_callbacks)
    self:load()
    if b_callbacks
    then
        self:bind_callbacks()
